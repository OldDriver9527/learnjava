...
public class GC {
     ...
     * 垃圾搜集算法
     * 引用计数
     * 对象中记录指向当前对象的指针个数，不常用
     * 根可达算法
     * 从根对象向下查找引用的对象，直到叶子对象为止，未被查到的对象就属于垃圾
     *
     ...
     * 进行两次遍历，产生碎片
     * ii.拷贝   copying
     * 将内存划分为两块，活动区间，空闲区间。利用根可达算法找到活动区间中存活的对象，将存活对象复制到空闲区间，之后修改引用地址。回收所有活动区间
     * 浪费一半内存，不产生碎片
     ...
     * 常见垃圾回收器
     * i.serial
     * 单线程进行ygc的垃圾回收器，清理时所有工作线程停止，即stw(stop the world)。较少使用
     * ii.serial old
     * 单线程进行fullgc的垃圾回收器，清理时所有工作线程停止。较少使用
     * iii.parallel scavenge ps
     * 多线程进行ygc的垃圾回收器，清理时所有工作线程停止。默认使用
     * iiii.parallel old po
     * 多线程进行fullgc的垃圾回收器，清理时所有工作线程停止。默认使用
     * iiiii.parnew
     * 进行ygc的垃圾回收器。对ps在某方面进行增强，配合cms使用
