...
public class JavaSyntax{
	 ...
	 *  \" 	双引号字符
	 * iiiii.字符串常量值
	 * 字符串常量值使用双引号标识的字符序列，每个字符串常量值都是String类的一个实例，存储在字符串常量池中
	 * iiiiii.jdk7添加特性
	 ...
	 *
	 * 数组 Array
	 * 存储数据的容器，数组是一个对象，通过数组可以调用Object类中所有方法，也可以将数组地址赋值给Object类型引用
	 * i.数组类型
	 ...
	 * 数组类型 数组名；
	 * iii.创建数组对象
	 * 通过数组创建表达式(new)，数组初始化器完成数组对象创建
	 * 数组创建表达式必须指定元素类型，数组维度，至少一个维度数组长度
	 * 数组初始化器使用{}封装所有元素，元素，分隔，只能在声明数组类型变量时使用
	 * iiii.数组存储异常
	 * 当数组元素类型为引用类型，运行时为元素赋值，若被赋予的值与元素类型不兼容，会抛出数组存储异常
	 * iiiii.数组类型成员
	 * 数组类型中包含被final修饰的实例变量length
	 * 从Object类中继承的成员
	 * clone方法
	 * iiiiii.数组Class对象
	 * 每个数组对象都对应一个Class对象，元素类型相同的数组对象共享同一个Class对象
	 * iiiiiii.数组操作
