...
public class MySql {
     ...
     * 线程获取到读共享锁后，当前线程只能进行读操作，其他线程可以进行读操作，进行写操作会阻塞
     * myisam中可使用lock table，unlock tables显式获取锁，不建议
     ...
     * 线程获取到行写锁后，当前线程可以对行进行操作，其他线程操作该行会阻塞
     * 线程获取到行读锁后，当前线程只能对行进行读操作，其他线程可以对行进行读操作，进行写操作会阻塞
