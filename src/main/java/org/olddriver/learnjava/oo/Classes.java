...
public class Classes{
     ...
     *
	 * 类修饰符
     * 类可以使用public，protected，private，abstract，static，final，strictfp修饰
	 ...
	 * 域修饰符
     * 域可以使用public，protected，private，static，final，transient，volatile修饰
	 * static域
	 ...
	 * 保证线程可见性，防止指令重排序
	 * 域继承
	 * 子类继承父类，父接口中所有具有访问权限的域，且域没有被子类中域隐藏
     * 歧义继承
	 * i.父类中域和父接口中域同名，会导致歧义继承，通过简单名引用该域编译无法通过
	 * ii.多个父接口中域同名，若域定义在一个接口中，不会导致歧义继承(接口A中定义域，接口B，C，D继承A)
     * 若域定义在多个接口中，会导致歧义继承，通过简单名引用该域编译不通过(域定义在接口B，C，D中)
     * 域隐藏
     * 子类中域会隐藏从父类，父接口中继承的同名域。
     * 隐藏域通过子类类型引用无法访问，但对象向上转型后可以访问
     * 域隐藏与static无关，与域类型无关
	 * 实例变量与局部变量区别
