...
public interface Interfaces {
	 ...
	 * 子接口继承多个父接口中同名字段，若该字段定义在同一父接口中，不会导致歧义继承(A接口中定义某个字段，BC接口继承A，D继承BC)
	 * 若该字段定义在多个父接口中，会导致歧义继承，通过简单名直接调用编译无法通过
	 * 子接口中字段会隐藏父接口中可访问的同名字段
	 * 字段初始化
	 * 字段必须在声明时进行显式初始化，但初始化表达式中不能出现当前字段及其后声明字段
     ...
     * 若该方法定义在不同接口中，该方法若是抽象方法，可编译通过，若该方法为默认方法，必须在子接口中对此方法进行重写
	 ...
	 * 若子接口继承AB两个接口中类型同名，B接口继承A接口，会导致歧义继承，子接口中通过简单名引用该类型编译无法通过
	 * 子接口可以继承多个独立父接口中同名成员类，成员接口，当该类型定义在同一父接口中不会导致歧义继承，
