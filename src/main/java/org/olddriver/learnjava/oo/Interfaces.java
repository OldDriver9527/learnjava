...
public interface Interfaces{
     ...
     * 域修饰符
	 * 域只能用public static final修饰，接口中域隐式使用public static final修饰
	 * 域初始化
     * 域必须在声明时进行显式初始化，但初始化器中不能出现当前域及其后声明域
     * 使用常量值进行初始化的域优先于接口中其他域进行初始化
	 * 域继承
	 * 子接口会继承父接口中域，只要该域未被子接口中域隐藏
     * 域歧义继承
	 * 多个父接口中域同名，若域定义在一个接口中，不会导致歧义继承；若域定义在多个接口中，会导致歧义继承，通过简单名直接调用编译无法通过
	 * 域隐藏
     * 子接口中域会隐藏父接口中可访问的同名域
	 *
	 * 方法声明
