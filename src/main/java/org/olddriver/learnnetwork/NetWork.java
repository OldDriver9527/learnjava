...
public class NetWork {
     ...
     *
     * 因特网
     * Internet，全球最大互联网。由若干ISP组成
     * 因特网由主机，通信链路，路由器，链路层交换机等因特网组件组成
     * 因特网是网络应用传输数据的基础设施。
     * 主机提供套接字接口，运行在主机上的程序通过socket api请求因特网将数据传输到目标主机的程序上
     *
     * 因特网划分
     * 网络边缘 所有连接在因特网上的主机组成网络边缘，提供共享资源
     * 接入网络 将主机连接到核心网络中的边缘路由器，可以是简单通信链路或复杂网络
     *         家庭一般通过电话线路，有线电视线路接入核心网络
     *         企业一般通过局域网(LAN)，无线局域网(WLAN)接入核心网络
     *         以太网是一个局域网
     * 网络核心 由路由器和链路组成，连接各个端系统，实现分组交换
     *
     * 通信链路和分组交换机移动数据方式
     * 分组交换
     * 路由器采用存储转发机制移动分组，在路由器完全接收分组数据后才会转发分组
     * 分组交换的问题
     * 延时和丢失
     * 排队延时 分组在链路队列中等待的时间
     * 处理延时 路由器处理分组的时间
     * 传输延时 路由器发送分组的时间，等于分组长度L/链路带宽R
     * 传播延时 分组在链路中传播的时间，等于传播距离/传播速度
     * 分组丢失
     * 分组在传播过程中，当路由器队列已满，分组会丢失
     * 电路交换
     * 主机传输数据过程中会占用路径中资源
     *
     * 协议分层
     ...
     * 传输层  传输层中分组称为报文段(segment)，提供可靠的在源进程到目标进程之间传输数据服务
     * 网络层  网络层中分组称为数据报，提供在源主机到目标主机之间传输数据的服务
     * 链路层  链路层中分组称为帧，提供在相邻结点间传输数据的服务
     * 物理层  提供将比特流转换为物理信号在相邻结点间传输的服务
     *
     * 封装
